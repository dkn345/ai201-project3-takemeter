...
## Baseline
The baseline model was created using Groq's llama-3.3-70b-versatile model in a zero-shot classification setting. I wrote a prompt containing the community name, label definitions, and one example for each label. The prompt instructed the model to classify each post as either analysis or opinion and to respond using only the label name.

The analysis label was defined as posts that explain game mechanics, lore, troubleshooting, or evidence-based information. The opinion label was defined as posts that primarily express personal preferences, criticism, praise, speculation, or subjective judgments. The prompt also included examples of both labels to help the model understand the distinction.

To collect baseline results, I ran the Groq classifier on every post in the 30-example test set. The model's predicted labels were compared against the ground-truth labels from my annotated dataset. I then calculated accuracy, precision, recall, and F1-score using the same evaluation pipeline that was used for the fine-tuned model. The baseline achieved an accuracy of 83.3% on the test set.
...
## Spec Reflection
The spec helped me by forcing me to define the label boundaries before training. This made me think through edge cases like posts that mix facts with opinions before annotating the full dataset. One way my implementation diverged from my original plan was that I modified my label definitions while collecting Reddit examples. I originally thought the distinction would be simpler, but many posts mixed gameplay/lore evidence with personal reactions, so I refined the rules to focus more on the post’s primary purpose.
